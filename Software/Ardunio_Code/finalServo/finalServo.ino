 ...
 * 
 * Description:
 *    This is Arduino code to run the Final Project servos.
 *    It recieves commands from a raspberry pi, as pin inputs
 *    and calls functions to move the robots servos.
 */
...
// Final demo stuff "Wave","Point","No","Neutral","Arms_up","Demo"
#define WAVE_PIN 1
#define POINT_PIN 2
#define NO_PIN 3
#define NEUTRAL_PIN 4
#define ARMS_UP_PIN 5
#define DEMO_PIN 6
...
*/
  ...
  pinMode(POINT_PIN, INPUT);
  pinMode(NO_PIN, INPUT);
  pinMode(NEUTRAL_PIN, INPUT);
  pinMode(ARMS_UP_PIN, INPUT);
...
*/
  ...
  int wave = digitalRead(WAVE_PIN);
  int point = digitalRead(POINT_PIN);
  int no = digitalRead(NO_PIN);
  int neutral = digitalRead(NEUTRAL_PIN);
  int armsUp = digitalRead(ARMS_UP_PIN);
